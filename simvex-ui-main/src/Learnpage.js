...
const ViewerEngineSVG = () => (
  <svg viewBox="0 0 380 300" fill="none">
    ...
    <text x="190" y="160" textAnchor="middle" fill="#fff" fontSize="16">3D 뷰어</text>
  </svg>
);
...
const QUIZ_DATA = [
  { question: "제트 엔진의 주요 구성 요소가 아닌 것은?", options: ["압축기", "연소실", "터빈", "프로펠러"], answer: 3 },
];
...
/*  LearnPage                                   */
/* ════════════════════════════════════════════ */
export default function LearnPage({ onHome, onStudy, selectedModel }) {
  ...
  const [chatMsgs, setChatMsgs] = useState([
    { role: "ai", text: "안녕하세요! 궁금한 점이 있으신가요?" }
  ]);
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      const answer =
        (data && (data.answer || data.message || data.content)) ||
      ...
      setChatMsgs(prev => [
        ...prev,
        { role: "ai", text: `오류: ${err.message || "요청 실패"}` },
      ]);
    } finally {
                ...
                {activeTab === "퀴즈" ? (
                  <div className="quiz-box">
                    ...
                    {!quizFinished ? (
                      <div className="quiz-question-box">
                        ...
                        <div className="quiz-options">
                          {QUIZ_DATA[quizIdx].options.map((opt, i) => (
                            <button key={i} className={`quiz-option${quizSelected === i ? " selected" : ""}`} onClick={() => setQuizSelected(i)} disabled={quizSubmitted}>
                              {opt}
                            </button>
                          ))}
                        </div>
                        {!quizSubmitted ? (
                          <button className="quiz-submit" onClick={submitQuiz} disabled={quizSelected === null}>제출</button>
                        ) : (
                      ...
                      <div className="quiz-final">
                        ...
                        <button className="quiz-restart" onClick={resetQuiz}>다시 풀기</button>
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="viewer-body-row">
                    ...
                    {showProductPanel && (
                      <div className="viewer-product">
                        ...
                        <div className="viewer-product-body">
                          <div className="viewer-product-model-title">
                            {selectedModel?.title || "모델 선택 필요"}
                          </div>
                    ...
                    <div className={`viewer-3d${(!showInfoPanel && !showProductPanel) ? " expanded" : ""}`}>
                      <div className="viewer-3d-content">
                    ...
                    {showInfoPanel && (
                      <div className="viewer-info">
                        ...
                        {partsLoading && (
                          <div className="viewer-parts-status">불러오는 중…</div>
                        )}

                        {partsErr && (
                          <div className="viewer-parts-status error">오류: {partsErr}</div>
                        )}

                        {!partsLoading && !partsErr && parts.length > 0 && (
                          <>
                            {/* 부품 썸네일 그리드 */}
                            ...
                            {/* 선택된 부품 상세 정보 */}
                            {selectedPart && (
                              <div className="viewer-part-detail">
                                <div className="viewer-part-detail-name">
                                  {selectedPart.meshName || selectedPart.name || "이름 없음"}
                                </div>
                                
                                {(selectedPart.description || selectedPart.desc) && (
                                  <div className="viewer-part-detail-section">
                                    <div className="viewer-part-detail-label">설명</div>
                                    <div className="viewer-part-detail-text">
                                      {selectedPart.description || selectedPart.desc}
                                    </div>
                                  </div>
                                )}
                                ...
                                {selectedPart.function && (
                                  <div className="viewer-part-detail-section">
                                    <div className="viewer-part-detail-label">기능</div>
                                    <div className="viewer-part-detail-text">
                                      {selectedPart.function}
                                    </div>
                                  </div>
                                ...
                                {selectedPart.material && (
                                  <div className="viewer-part-detail-section">
                                    <div className="viewer-part-detail-label">재질</div>
                                    <div className="viewer-part-detail-text">
                                      {selectedPart.material}
                                    </div>
                                  </div>
                                ...
                                {selectedPart.structure && (
                                  <div className="viewer-part-detail-section">
                                    <div className="viewer-part-detail-label">구조</div>
                                    <div className="viewer-part-detail-text">
                                      {selectedPart.structure}
                                    </div>
                                  </div>
                                ...
                                {selectedPart.type && (
                                  <div className="viewer-part-detail-section">
                                    <div className="viewer-part-detail-label">타입</div>
                                    <div className="viewer-part-detail-text">
                                      {selectedPart.type}
                                    </div>
                                  </div>
                        ...
                        {!partsLoading && !partsErr && parts.length === 0 && (
                          <div className="viewer-parts-status">
                            부품 정보가 없습니다.
                          </div>
                        )}
                      </div>
                    ...
                    {memos.map((memo, idx) => (
                      <div key={idx} className="memo-note">
                        ...
                        <div className="memo-note-body">
                          ...
                          <textarea className="memo-note-content-input" placeholder="내용..." value={memo.content} onChange={e => updateMemo(idx, "content", e.target.value)} />
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {expandedMemo !== null && memos[expandedMemo] && (
        <div className="memo-modal-overlay" onClick={() => setExpandedMemo(null)}>
          <div className="memo-modal" onClick={e => e.stopPropagation()}>
            ...
            <textarea className="memo-modal-content" placeholder="내용..." value={memos[expandedMemo].content} onChange={e => updateMemo(expandedMemo, "content", e.target.value)} />
          </div>
        </div>
      )}
    </>
